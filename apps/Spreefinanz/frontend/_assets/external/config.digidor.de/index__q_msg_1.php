...
<html lang="de" class="module_login moduleaction_index  backendmenu_modern">
<head>
...
digidor.challengeToken.next( 'challenge_token=aa45f98a17a813fc5641dfd3ff1c253ae9ee92327c649a2d99' );
...
<form id="form_login" method="post" action="index.php?module=login&action=logon&noout=1" autocomplete="off" style="margin: 100px auto 0; max-width: 420px;">
	...
	$( document ).ready( function() {
		...
		$( '#form_login' ).on( 'submit', function() {
			...
			if( digidor.login.isFriendlyCaptchaReady() ) {
				
				digidor.wait.show();
				
				$( '#tf_password' ).val( encodeURIComponent( $( '#tf_password' ).val() ) );
    ...
    <label style="display: block; padding-bottom: 5px;" for="tf_user">Benutzername:</label>
	<input name="tf_user" type="text" id="tf_user" class="input2014" autocomplete="off" value=""/>
	
	<div style="height: 10px; width: 100%;"></div>
	
	<label style="display: block; padding-bottom: 5px;" for="tf_password">Passwort:</label>
	<input name="tf_password" type="password" id="tf_password" class="input2014" autocomplete="off"/>
	
	<div class="frc-captcha" data-sitekey="FCMSEQPNJQ9M8O3T" data-lang="de" data-start="focus" data-puzzle-endpoint="https://eu-api.friendlycaptcha.eu/api/v1/puzzle"></div>	
	<div style="height: 10px; width: 100%;"></div>
	

	
	<div style="margin: 20px 0 10px; color: #cc0000; font-weight: bold;">Benutzername oder Passwort falsch.</div>
	
		

	<input name="digidor_data" type="hidden" value=""/>
	<input name="prefilled_user" type="hidden" value=""/>
	
	<div style="text-align: right;">
		<button class="cm_actionbutton" type="submit" >anmelden</button>
	</div>

</form>	
	
<div style="margin: 10px auto 100px; max-width: 420px;">

			
		<p><a href="index.php?module=login&amp;action=sendpassword">Ich habe mein Passwort vergessen</a></p>
		<p style="font-size: 12px; color: #999; border-top: 1px solid #999; padding-top: 10px; margin-top: 50px;">Hinweis: zur Anmeldung m&uuml;ssen Cookies aktiviert sein.</p>
...
<script>
    ...
    digidor.backend.init( 'login', 'index', '' );
